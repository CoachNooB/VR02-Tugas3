...
public class HalimahEvacuationManager : MonoBehaviour
{
    // UI References
    ...
    void Start()
    {
        ...
        HalimahTimerManager timerMgr = FindAnyObjectByType<HalimahTimerManager>();
    ...
    {
        Debug.Log("[HalimahEvacuationManager] ID Card button clicked!");
    ...
    {
        Debug.Log("[HalimahEvacuationManager] Alarm button clicked!");
        if (isEvacuationComplete || timeRemaining <= 0) return;
    ...
    {
        Debug.Log("[HalimahEvacuationManager] Door button clicked!");
        if (isEvacuationComplete || timeRemaining <= 0) return;
        ...
        if (!isDoorOpened)
        {
            ...
            HalimahDoorController door = FindAnyObjectByType<HalimahDoorController>();
    ...
    {
        Debug.Log("[HalimahEvacuationManager] Exit button clicked!");
        if (isEvacuationComplete || timeRemaining <= 0) return;
    ...
    private void ChangeStationColor(GameObject station, Color color)
    {
        ...
        HalimahStationClickEffect effect = station.GetComponent<HalimahStationClickEffect>();
        if (effect != null)
        {
