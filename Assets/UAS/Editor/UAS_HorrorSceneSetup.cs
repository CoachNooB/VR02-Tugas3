...
public class UAS_HorrorSceneSetup : EditorWindow
{
    ...
    public static void SetupHauntedHouse()
    {
        ...
        // Find Floor, Cealing, Walls and apply material
        ApplyMaterialToMesh("Floor", darkMat);
        ApplyMaterialToMesh("Cealing", darkMat);
        ApplyMaterialToMesh("Wall_Left", darkMat);
        ApplyMaterialToMesh("Wall_Right", darkMat);
        ApplyMaterialToMesh("Wall_Back", darkMat);
        ApplyMaterialToMesh("Wall_Front", darkMat);

        // 3. Create Spooky Point Lights in the Room
        ...
        // 4. Create World Space Canvas UI if not present
        ...
        // 6. Setup some Interactables & Trigger Zones in the room
        ...
        EditorUtility.DisplayDialog("UAS Horror Setup", "Haunted House Scene setup completed successfully!\n\nCheck the scene to see the dark atmosphere, spooky red/green lights, and the World Space UI.", "OK");
    }

    private static void ApplyMaterialToMesh(string name, Material mat)
    {
        ...
        if (obj != null)
        {
            ...
            if (mr != null)
            {
                ...
                Debug.Log($"Applied dark material to {name}");
            }
        }
    }
