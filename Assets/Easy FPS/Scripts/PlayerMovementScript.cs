...
public class PlayerMovementScript : MonoBehaviour {
	...
	*/
	...
	/*
     * Method yang dipanggil jika animasi serangan melee dipicu
     */
    IEnumerator MeeleAttackWeaponHit(){
        if (Physics.Raycast (ray1, out hitInfo, 2f, ~ignoreLayer) || Physics.Raycast (ray2, out hitInfo, 2f, ~ignoreLayer) || Physics.Raycast (ray3, out hitInfo, 2f, ~ignoreLayer)
            ...
            if (hitInfo.transform.tag == "Dummie") {
                // Ambil komponen MonsterHealth dari objek yang terkena raycast atau objek induknya
                MonsterHealth monster = hitInfo.transform.GetComponentInParent<MonsterHealth>();
                if (monster == null) {
                    monster = hitInfo.transform.GetComponent<MonsterHealth>();
                }

                // Jika script komponen ditemukan, kirim damage
                if (monster != null) {
                    monster.TakeDamage();
                } else {
                    print("Hit Dummie tapi script MonsterHealth tidak ditemukan!");
                }
