...
public class GunInventory : MonoBehaviour {
    ...
    // === SISTEM PELURU BERBASIS DICTIONARY ===
    private Dictionary<string, float> savedBulletsIHave = new Dictionary<string, float>();
    private Dictionary<string, float> savedBulletsInTheGun = new Dictionary<string, float>();

    // --- SISTEM NOTIFIKASI OVERLAY MONSTER ---
    [Header("Monster Quest Settings")]
    public TMPro.TextMeshProUGUI notificationUIText; // Seret komponen Text UI TMP kamu ke sini di Inspector
    
    [HideInInspector] 
    public int monsterPoints = 0; 
    
    private int totalMonstersRemaining = 0;

    void Start() {
        // Otomatis menghitung semua monster ber-tag Dummie yang ada di dalam Map saat game dimulai
        GameObject[] monsters = GameObject.FindGameObjectsWithTag("Dummie");
        totalMonstersRemaining = monsters.Length;

        UpdateMonsterNotification();
    }

    public void AddMonsterPoint() {
        monsterPoints += 1;
        totalMonstersRemaining--; // Kurangi sisa monster yang harus dicari

        UpdateMonsterNotification();
    }

    // === FOKUS PERUBAHAN: Ditambahkan sistem pop-up timer ===
    void UpdateMonsterNotification() {
        if (notificationUIText != null) {
            if (totalMonstersRemaining > 0) {
                notificationUIText.text = "Cari " + totalMonstersRemaining + " monster lagi!";
            } else {
                notificationUIText.text = "Semua monster telah ditemukan!";
            }

            // Hentikan hitungan mundur yang lama (jika ada), lalu jalankan yang baru selama 3 detik
            StopCoroutine("HideNotificationAfterDelay"); 
            StartCoroutine("HideNotificationAfterDelay", 3f); 
        }
    }

    // Fungsi pembantu untuk menyembunyikan teks otomatis setelah beberapa detik
    IEnumerator HideNotificationAfterDelay(float delay) {
        notificationUIText.gameObject.SetActive(true);
        yield return new WaitForSeconds(delay);
        notificationUIText.gameObject.SetActive(false);
    }

    // Fungsi bantuan untuk menyimpan peluru (Akan dipanggil dari GunScript)
    public void SaveWeaponAmmo(string weaponName, float iHave, float inTheGun) {
        savedBulletsIHave[weaponName] = iHave;
        savedBulletsInTheGun[weaponName] = inTheGun;
    }

    // Fungsi bantuan untuk mengambil data peluru (Akan dipanggil dari GunScript)
    public bool LoadWeaponAmmo(string weaponName, out float iHave, out float inTheGun) {
        if (savedBulletsIHave.ContainsKey(weaponName) && savedBulletsInTheGun.ContainsKey(weaponName)) {
            iHave = savedBulletsIHave[weaponName];
            inTheGun = savedBulletsInTheGun[weaponName];
            return true;
        }
        iHave = 0;
        inTheGun = 0;
