...
/*
     * Diambil saat senjata baru pertama kali muncul / dipakai kembali
     */
    void Start() {
        if (pmS) {
            GunInventory inventory = pmS.GetComponent<GunInventory>();
            
            float loadedIHave, loadedInTheGun;
            // Cek ke inventory, apakah nama senjata ini sudah pernah punya catatan peluru?
            if (inventory && inventory.LoadWeaponAmmo(gameObject.name, out loadedIHave, out loadedInTheGun)) {
                bulletsIHave = loadedIHave;
                bulletsInTheGun = loadedInTheGun;
            }
        }
    }

    /*
     * Berjalan otomatis tepat sebelum senjata ini dihancurkan saat ganti senjata
     */
    void OnDestroy() {
        if (pmS) {
            GunInventory inventory = pmS.GetComponent<GunInventory>();
            ...
            if (inventory) {
                // Simpan sisa peluru spesifik menggunakan nama GameObject senjata ini
                inventory.SaveWeaponAmmo(gameObject.name, bulletsIHave, bulletsInTheGun);
            }
        }
    }
	...
	*/
    ...
    public void AddMonsterPoint() {
        monsterPoints += 1;
        Debug.Log("Poin Monster Bertambah! Total Poin Saat Ini: " + monsterPoints);
    }
	void OnGUI(){
		if(!HUD_bullets){
			try{
